...
describe('Feathers Memory Service', () => {
    ...
    const app = feathers();
    app.use('people', new LowDBService({ events }));
    app.use('people-customid', new LowDBService({
        id: 'customid',
        events,
    }));
    ...
    it('patch record with prop also in query', async () => {
        app.use('animals', new LowDBService({ multi: true }));
    ...
    it('allows to pass custom find and sort matcher', async () => {
        ...
        app.use('matcher', new LowDBService({
            matcher() {
                matcherCalled = true;
                return function () {
                    return true;
